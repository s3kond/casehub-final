...
export default function App() {
  ...
  const fetchOrders = useCallback(async (tgId: string) => {
    if (!tgId) return;
    try {
      const res = await fetch(`${API_URL}/api/orders/${tgId}`);
      const data = await res.json();
  ...
  useEffect(() => {
    ...
    const init = async () => {
      ...
      const tgUser = tg?.initDataUnsafe?.user || { id: 1822541018, username: 'vovchik', first_name: 'Vovchik' };
      try {
        const auth = await fetch(`${API_URL}/api/auth`, {
          method: 'POST',
      ...
      <div className="min-h-screen bg-[#0a0a0a] text-white pb-32">
        ...
        {selectedProduct && (
          <div className="fixed inset-0 z-[1000] bg-black/80 backdrop-blur-xl flex items-end justify-center">
            ...
            <div className="relative w-full max-w-lg bg-[#0a0a0a] rounded-t-[3rem] border-t border-white/10 p-6 overflow-y-auto max-h-[90vh]">
               ...
               <p className="text-blue-500 font-black text-xl mb-6">${selectedProduct.price_usd}</p>
               <button onClick={() => { 
